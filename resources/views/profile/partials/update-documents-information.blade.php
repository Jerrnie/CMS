<section>
    <header>
        ...
        <p class="mt-1 text-sm text-gray-600">
            {{ __("Attachment Summary Information") }}
        </p>
    </header>
    <form method="POST" action="{{ route('profile-attachement-info.store') }}" enctype="multipart/form-data">
        @csrf
    <div class="mt-4">
    ...
    <div class="mt-4">
        <x-input-label class="required" for="category" :value="__('Category')" />
        <select id="category" name="category" class="block mt-1 w-full" required>
            <option value="">Select a category</option>
            @foreach ($documentCategories as $id => $name)
